...
//  Lab2
//
//  Created by Kaelen Guthrie on 2/18/18.
...
class SearchResultsTableViewController: UITableViewController, UISearchResultsUpdating {
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        filteredAssignments.removeAll(keepingCapacity: true) //removes all elements
        if searchString?.isEmpty == false {
            ...
            let matches = allAssignments.filter(searchfilter)
            filteredAssignments.append(contentsOf: matches)
        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = filteredAssignments[indexPath.row]
        return cell
    }
